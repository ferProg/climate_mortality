...
def load_eea_island(eea_dir: Path, island: str,
    ...
    if poll_col:
        ...
        print(f"    EEA pollutant codes found: {raw[poll_col].unique().tolist()}")
        print(f"    EEA rows after year filter: {len(raw)}")
        print(f"    EEA mapped pollutants: {raw["_poll_std"].value_counts().to_dict()}")
        raw = raw[raw["_poll_std"].isin(POLLUTANTS)].copy()
        print(f"    EEA rows after pollutant filter: {len(raw)}")
        if raw.empty:
            print(f"    WARNING: no matching pollutant rows for island={island}")
            return pd.DataFrame()
        daily = (
...
def load_excel_island_year(excel_dir: Path, island: str, year: int) -> pd.DataFrame:
    ...
    for sheet in sheets:
        try:
            ...
            daily["pm10_ok"] = daily["PM10"].notna().astype(bool)
            daily["station"] = sheet
    ...
    combined = (
        combined[combined["pm10_ok"] == True]
