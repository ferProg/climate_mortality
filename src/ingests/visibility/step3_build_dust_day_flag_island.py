...
DEFAULT_OUT_DIR = Path("data/interim/visibility/step3_daily")
...
def run_step3_build_island_flags(
    ...
    Output: island daily with:
      - low_vis_confirmed_day (visibility-based, Rule C)
      - low_vis_possible_day (visibility missing but very dry)
      - low_viscalima_any_day = confirmed OR possible
    """
    ...
    df["low_vis_confirmed_station_day"] = confirmed.astype(int)
    df["low_vis_possible_station_day"] = possible.astype(int)

    island_daily = (df.groupby("date", as_index=False)
                      .agg(
                          confirmed_airports=("low_vis_confirmed_station_day", "sum"),
                          possible_airports=("low_vis_possible_station_day", "sum"),
                          airports_obs=("station", "nunique"),
                          vis_min_m=("vis_m", "min"),
                          rh_min_pct=("rh_pct", "min"),
                      ))

    # Island-level positive if at least one selected airport is positive that day.
    island_daily["low_vis_confirmed_day"] = (island_daily["confirmed_airports"] >= 1).astype(int)
    island_daily["low_vis_possible_day"] = (island_daily["possible_airports"] >= 1).astype(int)
    island_daily["low_vis_any_day"] = ((island_daily["low_vis_confirmed_day"] == 1) |
                                      (island_daily["low_vis_possible_day"] == 1)).astype(int)

    island_daily["island"] = island

    if out_name is None:
        out_name = f"low_vis_proxy_daily_{island}.parquet"
